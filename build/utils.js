...
exports.cssLoaders = function (options) {
  ...
  function generateLoaders (loader, loaderOptions) {
    //通过usePostCss来标明是否使用了postcss
    const loaders = options.usePostCSS ? [cssLoader, postcssLoader] : [cssLoader]

    //如果指定了具体的Loader的名称
    if (loader) {
      //想loader的数组中添加该loader对应的加载器
      //一个重要的地方就是。一个数组中的loader加载器，是从右向左执行的
      loaders.push({
        //loader加载器的名称
    ...
    //如果明确指定了需要提取静态文件，则使用extractTextPlugin.extract({})来包裹我们的各种css处理器
    if (options.extract) {
      ...
      //如果没有提取行为，则最后再使用vue-style-loader
